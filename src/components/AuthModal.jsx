      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 pb-0">
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 hover:bg-gray-200 transition-colors text-sm">
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 mb-4">
              {[['login','Sign In'],['register','Create Account']].map(([t, l]) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === t ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  {l}
                </button>
              ))}
            </div>
        ...
        <div className="p-6 pt-2">
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div className="relative">
                <input className="input pr-11" type={showPwd ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPass(e.target.value)} />
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister} className="space-y-3">
              ...
              <div className="relative">
                <input className="input pr-11" type={showPwd ? 'text' : 'password'} placeholder="Password (min 8 characters) *" value={password} onChange={e => setPass(e.target.value)} />
                <button type="button" onClick={() => setShowPwd(s => !s)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                  <i className={`fas ${showPwd ? 'fa-eye-slash' : 'fa-eye'} text-sm`} />
                </button>
              ...
              <p className="text-xs text-center text-gray-400">
                ...
                <button type="button" className="text-primary font-semibold hover:underline" onClick={() => setTab('login')}>Sign in</button>
              </p>
            </form>
          )}
          ...
          {tab === 'forgot' && (
            <form onSubmit={handleReset} className="space-y-3">
              <p className="text-sm text-gray-500 mb-2">Enter your email and we will send you a reset link.</p>
              <input className="input" type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} />
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3">
              ...
              <button type="button" className="w-full text-sm text-primary font-semibold hover:underline" onClick={() => setTab('login')}>
                <i className="fas fa-arrow-left text-xs mr-1" /> Back to Sign In
              </button>
            </form>
          )}
        </div>
