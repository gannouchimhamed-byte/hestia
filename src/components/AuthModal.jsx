...
export default function AuthModal({ open, tab: initialTab, onClose }) {
  ...
  async function handleLogin(e) {
    ...
    else { toast('Welcome back!', 'success'); onClose() }
  }
        ...
        <div className="p-6 pb-0">
          <div className="flex items-center justify-between mb-4">
            <div className="font-display text-2xl text-primary flex items-center gap-2">
              <span className="text-accent">⌂</span> Hestia
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 hover:bg-gray-200 transition-colors">✕</button>
          </div>
          <p className="text-sm text-gray-400 mb-4">Tunisia's premier real estate platform</p>
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 mb-4">
              {TABS.map(t => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === t.id ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  {t.label}
                </button>
              ))}
            </div>
        ...
        <div className="p-6 pt-2">
          ...
          {tab !== 'forgot' && (
            <button onClick={signInWithGoogle}
              className="w-full flex items-center justify-center gap-3 py-3 border border-gray-200 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-all mb-3">
          ...
          {success && <div className="mb-3 p-3 bg-emerald-50 text-emerald-700 text-sm rounded-xl">✓ {success}</div>}
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div className="relative">
                <input className="input pr-12" type={showPwd ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPass(e.target.value)} />
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm" onClick={() => setShowPwd(s => !s)}>{showPwd ? '🙈' : '👁'}</button>
              </div>
              <div className="text-right"><button type="button" className="text-xs text-primary font-semibold" onClick={() => setTab('forgot')}>Forgot password?</button></div>
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3">
                {loading ? <span className="animate-spin">⏳</span> : '→'} Sign In
              </button>
              <p className="text-xs text-center text-gray-400">No account? <button type="button" className="text-primary font-semibold" onClick={() => setTab('register')}>Create one free</button></p>
            </form>
          )}
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister} className="space-y-3">
              ...
              <div className="relative">
                <input className="input pr-12" type={showPwd ? 'text' : 'password'} placeholder="Password (min 8 chars) *" value={password} onChange={e => setPass(e.target.value)} />
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm" onClick={() => setShowPwd(s => !s)}>{showPwd ? '🙈' : '👁'}</button>
              </div>
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3">
                {loading ? <span className="animate-spin">⏳</span> : '+'} Create Account
              </button>
              <p className="text-xs text-center text-gray-400">Already have an account? <button type="button" className="text-primary font-semibold" onClick={() => setTab('login')}>Sign in</button></p>
            </form>
          )}
          ...
          {tab === 'forgot' && (
            <form onSubmit={handleReset} className="space-y-3">
              ...
              <button type="button" className="w-full text-sm text-primary font-semibold" onClick={() => setTab('login')}>← Back to Sign In</button>
            </form>
          )}
        </div>
      </div>
